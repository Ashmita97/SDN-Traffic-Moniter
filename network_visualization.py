...
TOPOLOGY_DB = '/home/ubuntu/ryu/ryu/app/SDN_Project/topo1.db'
SW_TABLE = 'switches'
LINKS_TABLE = 'topo_connections'
HOSTS_TABLE = 'host_connections'
#TOPOLOGY_IMAGE = 'static/topology.png'
TOPOLOGY_IMAGE = '/home/ubuntu/ryu/ryu/app/SDN_Project/topo5.png'
PORT_UP = 'UP'
PORT_DOWN = 'DOWN'
...
def draw_nodes(G, switches, hosts):
    ...
    nx.draw_networkx_nodes(G, pos, node_shape = 'o',
        nodelist = switches, node_color = 'y', node_size = 10000,
        edgecolors = 'black')
    nx.draw_networkx_nodes(G, pos, node_shape = 's', 
        nodelist = hosts, node_color = 'blue', node_size = 10000,
        edgecolors = 'black', alpha = 0.2)

def draw_edges(G, links, hosts_switches_links, l, new_or_l, new_r_l):
    ...
    up_links, red_links, orange_links = check_link_status(links, l, new_or_l, new_r_l)
    nx.draw_networkx_edges(G, pos, edgelist = up_links, edge_color = 'g')
    nx.draw_networkx_edges(G, pos, edgelist = red_links, edge_color = 'r')
    nx.draw_networkx_edges(G, pos, edgelist = orange_links, edge_color = 'orange')
    nx.draw_networkx_edges(G, pos, edgelist = hosts_switches_links,
    edge_color = 'g')

def check_link_status(links, l, new_or_l, new_r_l):
    ...
    for link in links:
    	first = link[0]
    	second = link[1]
    	if first in new_or_l and second in new_or_l:
    		orange_links.append(link)
    	if first in new_r_l and second in new_r_l:
    		red_links.append(link)
    	if link not in red_links and link not in orange_links:
    		up_links.append(link)

    return up_links, red_links, orange_links
...
def draw_topology(switches, links, hosts, hosts_switches_links, cli, l, new_or_l, new_r_l):
    ...
    draw_edges(G, links, hosts_switches_links, l, new_or_l, new_r_l)
    draw_labels(G, links)
    plt.axis('off')
...
def visualize_topology(cli, l, new_or_l, new_r_l):
    ...
    if switches or links or hosts:
        draw_topology(switches, links, hosts, hosts_switches_links, cli, l, new_or_l, new_r_l)
